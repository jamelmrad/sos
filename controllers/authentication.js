...
exports.LoginMobile = async (req, res) => {
  try {
    ...
    if (!phone || !password) { // if we didn't get email or password
      return res.send({ message: 'All fields are required' })
    }
    const user = await User.findOne({ phone }); // retrieving the user
    if (!user) { // if user not found we send message error
      return res.send({ message: 'Incorrect password or phone' })
    }
    const auth = bcrypt.compareSync(password, user.password) // verifying password
    if (!auth) { //if password is incorect
      return res.send({ message: 'Incorrect password or phone' })
...
exports.LogoutMobile = async (req, res) => {
  try {
    ...
    if (!phone) { // if we didn't get email or password
      return res.send({ message: 'Phone are required' })
    }
    const user = await User.findOne({ phone }); // retrieving the user
    if (!user) { // if user not found we send message error
      return res.send({ message: 'Incorrect phone' })
    }
    user.token = ''
    user.save()
    res.send({ message: 'User logged out succesfully.' })
  } catch (error) {
    res.status(500).send({ message: error.message });
  }
}
